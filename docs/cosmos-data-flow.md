...
| Actor | When |
|-------|------|
| [AutoNodeStatus](#autonodestatus) | Observe-only: while the cluster is not being deleted and the HostedCluster has been observed via its ReadDesire, mirrors `HostedCluster.status.autoNode` (NodeCount / NodeClaimCount / VCPUs), the distilled `AutoNodeEnabled` condition (Enabled / Condition), and a failed AutoNode ApplyDesire delivery (DeliveryCondition) here, and clears the whole value back to nil once none of those has ever been observed |
